"use client";
import { socialPostsData } from "@/src/constants/news";
...
const SocialPostSliderCard: React.FC<{
  data: {
    id: number;
    link: string;
    title: string;
...
export default function SocialPostSlider() {
  return (
    <div
      data-aos="fade-up"
      className="max-w-[1750px] py-10 max-[1750px]:max-w-[90vw] max-sm:max-w-[93vw] relative mx-auto "
    >
      <Swiper
        slidesPerView={1}
        navigation={{
        ...
        className={`mySwiper max-w-[90%]  !flex items-center justify-center`}
      >
        {socialPostsData.map((data: any) => (
          <SwiperSlide
            key={data.id}
            className={` !flex items-center justify-center `}
          >
            <SocialPostSliderCard data={data} />
          </SwiperSlide>
        ))}
      </Swiper>

      <button
        className={`prevSocial group absolute z-40 top-1/2 -translate-y-full left-0 disabled:cursor-not-allowed`}
      >
        <svg
          width="30"
          height="25"
      ...
      <button
        className={`nextSocial group absolute z-40 top-1/2 -translate-y-full right-0 disabled:cursor-not-allowed`}
      >
        <svg
          width="30"
          className="group-disabled:opacity-40"
